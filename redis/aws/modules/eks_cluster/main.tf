...
locals {
  ...
  node_pool_tags = merge(var.tags, {
    Name       = "${var.name}-node-pool"
    managed_by = "terraform"
  })
  ebs_csi_driver_configuration_values = {
    controller = {
...
resource "aws_eks_node_group" "worker_nodes" {
  ...
  depends_on = [
    aws_launch_template.worker_nodes,
...
resource "aws_eks_addon" "aws_ebs_csi_driver" {
  ...
  configuration_values = jsonencode(local.ebs_csi_driver_configuration_values)
